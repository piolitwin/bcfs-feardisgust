---
title: <font size="5">**Perceptual access to homeostatically relevant visual stimuli" (https://osf.io/dyw64/) - Disgust**</font> 
...
setwd("C:/Users/pioli/OneDrive/Pulpit/PRELUDIUM/Preludium 2/data/Disgust")
...
# Compare aversion levels with the fear conditioning procedure (Exp. 1)

# Working directory to Fear - remember to set your own wd path
setwd("C:/Users/pioli/OneDrive/Pulpit/PRELUDIUM/Preludium 2/data/Fear")

dfsparfear = list.files(pattern = "*parameters.csv")
ldfparfear = lapply(dfsparfear, read.csv)

dbparfear = as.data.frame(ldfparfear[1])
for (i in 2:length(ldfparfear)) {
  dat = as.data.frame(ldfparfear[i])
  cols = intersect(colnames(dat), colnames(dbparfear))
  dbparfear = rbind(dbparfear[, cols], dat[, cols])
  
}

avcompdf = rbind(dbpar[, c(1, 5, 6)], dbparfear[, c(1, 5, 6)])
avcompdf$UCS = as.factor(avcompdf$UCS)

avcompdf %$% t.test(Aversion ~ UCS, paired = FALSE, var.equal = TRUE)
...
# Working directory back to Disgust - remember to set your own wd path
setwd("C:/Users/pioli/OneDrive/Pulpit/PRELUDIUM/Preludium 2/data/Disgust")
...
# Two chance performers detected in the localization task ("B25" & "B50")
...
# Two additional participants to be removed from the localization-related analyses due to overperforming ("B8" & "B18")
unique(locperf$ID[locperf$meanlocacc >= 95])

# 4 participants to be removed altogether ("B8", "B18", "B25", "B50")
...
locacc %>% subset(!ID%in% outliersloc) %$% 
  t.test(Accuracy ~ conditioning, paired = TRUE, alternative = "less")

# two-sided exploratory analysis
locacc %>% subset(!ID%in% outliersloc) %$% 
  t.test(Accuracy ~ conditioning, paired = TRUE, alternative = "two.sided")
...
write.csv(subset(locaccoutput, !ID%in%outliersloc), "C:\\Users\\pioli\\OneDrive\\Pulpit\\PRELUDIUM\\Preludium 2\\Data\\Disgustloc.csv",  row.names=FALSE)
...
# Four chance performers detected in the discrimination task
...
# No overperformers in the discrimination task
unique(idperf$ID[idperf$meanidacc >= 95])

outliersid = c(unique(dp2$id[dp2$lng < 0]), unique(idperf$ID[idperf$meanidacc >= 95]))

# Four participants to be removed altogether ("B2", "B16", "B25", "B28")
outliersid
...
write.csv(subset(locaccoutput, !ID%in%outliersloc), "C:\\Users\\pioli\\OneDrive\\Pulpit\\PRELUDIUM\\Preludium 2\\data\\Disgustid.csv",  row.names=FALSE)
...
# "B27" is a large outlier, probably misunderstood id task (already detected before)
# Visual inspection also reveals another outlier who probably misunderstood id task ("B16", also detected before)

ggplot(perf,aes(meanlocacc, meanidacc, label = ID)) + geom_point() + geom_smooth(method = lm) +
  geom_text(nudge_y = -1.5, check_overlap = TRUE) 

# Also "B25" is similarly chance performing in both tasks. 
# Let's run the analysis with participants "B16", "B25", and "B28" excluded

outliers = c("B16", "B28", intersect(outliersloc, outliersid))
...
# MAIN PLOT - task-specific outliers excluded
...
plotframe %>% ggplot(aes(x = plotfactor, y = Accuracy, fill = conditioning)) +
  ...
  geom_boxplot(width = .12, outlier.shape = NA, alpha = 0.6, size = 0.8, 
...
# Working directory - remember to set your own wd path
setwd("C:/Users/pioli/OneDrive/Pulpit/PRELUDIUM/Preludium 2/data/Disgust/SCR")
...
# 3 outliers according to this criterion 
unique(dbscr$ID[dbscr$SCR_standardized < 0])

scroutliers = unique(dbscr$ID[dbscr$SCR_standardized < 0])
...
# All variables violate the normality assumption
...
dbscr %>% subset(!ID%in% scroutliers & !ID%in% outliersloc) %$% 
  cor.test(differlocrec, SCR_standardized, method = "kendall", alternative = "two.sided")

# Correlation between SCR and conditioning effect - discrimination

dbscr %>% subset(!ID%in% scroutliers & !ID%in% outliersid) %$% 
  cor.test(differidrec, SCR_standardized, method = "kendall", alternative = "two.sided")
...
plot1 = dbscrplot %>% subset(!ID%in% outliersloc) %>% ggplot(aes(differlocrec, SCR_standardized, color = coneffloc)) +
  ...
  geom_smooth(data = dbscrplot %>% subset(!ID%in% outliersloc), aes(fill = coneffloc), alpha = 0.2, method = "glm", fullrange = T, show.legend = F) + theme_classic() +
...
plot2 = dbscrplot %>% subset(!ID%in% outliersid) %>% ggplot(aes(differidrec, SCR_standardized, color = coneffid)) +
  ...
  geom_smooth(data = dbscrplot %>% subset(!ID%in% outliersid), aes(fill = coneffid), alpha = 0.2, method = "glm", fullrange = T, show.legend = F) + theme_classic() +
  scale_color_manual(values=c(darken("palevioletred2", amount = 0.1), darken("palevioletred2", amount = 0.1))) +
  scale_fill_manual(values=c(darken("palevioletred2", amount = 0.1), darken("firebrick2", amount = 0.05))) +
  scale_y_continuous(limits= c(0, 2.5), breaks = c(0, 0.5, 1, 1.5, 2, 2.5)) +
  labs(x = "Difference in % accuracy in favor of CS+" , y = "Mean standardized SCR to US") + 
